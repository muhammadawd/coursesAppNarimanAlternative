            ...
            <div class="col-lg-12 col-md-12 col-sm-12 pt-4 pb-4">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page" @click.prevent>{{$t('all_faculties')}}
                        </li>
                    </ol>
                </nav>
            </div>
                ...
                <div class="dashboard_container">
                    <div class="row">
                        <div class="col-12 pt-3">
                            <h2 class="font-weight-bold fun_font text-center">{{$t('all_faculties')}}</h2>
                        </div>
                    </div>
                            ...
                            <ul>
                                <li class="list-inline-item">
                                    <button class="btn btn-danger-light"
                                            ...
                                            @click="$router.push({name:'add_faculty'})">
                                        <i class="ti-plus"></i>
                                        {{$t('add')}}
                                    </button>
                                </li>
                                    ...
                                    <td>
                                        <div class="btn-group direction-inverse">
                                            <button class="btn custom_btn btn-primary-light"
                                                    v-if="$helper.hasAccessPermission('delete-university')"
                                                    @click="deleteCategory(category.id)">
                                                {{$t('delete')}}
                                            </button>
                                            <button class="btn custom_btn btn-success-light"
                                                    v-if="$helper.hasAccessPermission('update-university')"
                                                    @click="$router.push({name:'edit_faculty',params:{id:category.id}})">
                                                {{$t('edit')}}
                                            </button>
                                        </div>
                                    </td>
